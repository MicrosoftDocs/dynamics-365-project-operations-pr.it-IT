---
title: Dettagli dell'intestazione per contratti basati su progetto
description: Questo articolo fornisce informazioni sui campi e sui contratti basati su progetto in Project Operations.
author: rumant
ms.date: 10/20/2020
ms.topic: article
...
ms.locfileid: "9824396"
---
# <a name="header-details-for-project-based-contracts"></a>Dettagli dell'intestazione per contratti basati su progetto
...
| Cliente | Scheda **Riepilogo** | Un riferimento alla società o al record di account del cliente. Quando un contratto viene creato da un'offerta, questo campo viene copiato dal campo corrispondente nel record dell'offerta. | La valuta del contratto di progetto viene impostata in modo predefinito in base alla valuta del cliente. La valuta può essere modificata prima di salvare il contratto. |
...
| Quantità registrata | Dettaglio di una riga basata su progetto | Tempo: tutta la quantità di tempo sui valori effettivi del costo del tempo per un ruolo nel progetto mappato a questa voce di contratto. Spese: tutte le quantità per la categoria di spesa nei valori effettivi dei costi di spesa nel progetto mappate alla voce di contratto. |
