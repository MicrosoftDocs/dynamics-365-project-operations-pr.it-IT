...
4. Se non si utilizza Spencer Low con il nome utente **spencerl**, è necessario aggiornare un file aggiuntivo. Aprire il file **DemoDataPreImportConfig.xml** e quindi trovare il tag **userstocreateandconfigure**. Aggiornare il tag **\<login\>** con il nome utente dell'utente Spencer Low. Per ulteriori informazioni, vedi le [Note tecniche](#technical-notes).
...
- **\<EntitiesToEnableScheduling\>** specifica quali entità devono essere abilitate per la prenotazione nella pianificazione di Microsoft Dynamics (noto anche come Universal Resource Scheduling).
